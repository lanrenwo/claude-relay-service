...
const CODEX_CLI_VERSION = process.env.CODEX_CLI_VERSION || '9.99.0'
...
const IMAGE_GENERATION_TOOL_FIELDS = [
  'action',
  'model',
  'quality',
  'size',
...
function buildImageGenerationTool(body = {}) {
  ...
  const sources = [
    { value: body, allowModelFields: false },
    {
      value: body && typeof body.image_generation === 'object' ? body.image_generation : null,
      allowModelFields: true
    },
    {
      value: body && typeof body.imageGeneration === 'object' ? body.imageGeneration : null,
      allowModelFields: true
    }
  ]

  for (const { value: source, allowModelFields } of sources) {
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    if (!headers['user-agent']) {
      headers['user-agent'] = `codex_cli_rs/${CODEX_CLI_VERSION}`
    }
    if (!headers['version']) {
